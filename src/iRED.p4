...
const ports_t LPBK_PORT                        = 128; // recirc port 0 on pipe 1
...
control SwitchIngress(
    ...
    RegisterAction<bit<16>, bit<16>, bool>(congest_port) read_congest_port = {
    ...
    apply {
        ...
        if (ig_intr_md.ingress_port == LPBK_PORT){
            ...
            if (ig_intr_md.ingress_port == 448) {
                ig_tm_md.ucast_egress_port = 137;
            }

            // 24/0 (40) TF1 -> 1/1(137) Luigi
            if (ig_intr_md.ingress_port == 440) {
                ig_tm_md.ucast_egress_port = 137;
            }
...
control Egress(
        ...
        Counter<bit<32>, bit<32>>(N_PORTS, CounterType_t.PACKETS) thresholdPkts;
        Counter<bit<32>, bit<32>>(N_PORTS, CounterType_t.PACKETS) totalPkts;
        Register<bit<16>, bit<16>>(N_PORTS) qdelay_classic;
        Register<bit<16>, bit<16>>(N_PORTS) qdelay_l4s;
        Register<bit<16>, bit<16>>(N_PORTS) switchLatency;
        ...
        RegisterAction<bit<16>, bit<16>, bit<16>>(switchLatency) switchLatencyAction = {
    ...
    apply {
        ...
        //* Only regular pkts *//a
        if (eg_intr_md.egress_port != LPBK_PORT){   
            
            //Counter of packets
            totalPkts.count((bit<32>)eg_intr_md.egress_port);

            //* Compute queue delay *//
            queue_delay = (value_t)eg_intr_md_from_prsr.global_tstamp - (value_t)hdr.bridge.ingress_global_tstamp;
            hdr.bridge.setInvalid();

            switchLatencyAction.execute((value_t)eg_intr_md.egress_port);
            ...
            bit<16> target_violations;
            target_violations = compute_target_violations.execute((bit<16>)eg_intr_md.egress_port);

            if (target_violations == 1){
                
                //Counter of packets
                thresholdPkts.count((bit<32>)eg_intr_md.egress_port);
